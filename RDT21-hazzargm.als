...
pred seq_match[s, s': NetState] {
	(one n: Seq_Num | ((s.packet).seq_num = n) and ((s'.packet).seq_num = n) and ((s'.reply).seq_num = n))
}

pred seq_adv[s, s': NetState] {
	((s.packet).seq_num = One) => (((s.reply).seq_num = One) and ((s'.packet).seq_num = Zero)) else
	(((s.packet).seq_num = Zero) and ((s.reply).seq_num = Zero) and ((s'.packet).seq_num = One))
}
